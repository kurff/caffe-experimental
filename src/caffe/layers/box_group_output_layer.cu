...
void BoxGroupOutputLayer<Dtype>::Forward_gpu(
  ...
  for (int j = 0; j < bottom_num_; j++) {
    ...
    cascade::DecodeGroupBBoxesGPU(nthreads,bbox_data,group_nums_[j],bbox_dim,width,height,
  ...
  vector<vector<cascade::BBox> > batch_boxes(num);
  ...
  for (int i = 0; i < num; i++) {
      ...
      for (int k = 0; k < group_num; k++) {
        ...
        for (int id = 0; id < spatial_dim; id++) {
          ...
          if (fg_score >= fg_thr_) {
            ...
            cascade::BBox bbox = cascade::InitBBox(bbox_pred_cpu[idx], bbox_pred_cpu[spatial_dim+idx], 
                    bbox_pred_cpu[2*spatial_dim+idx], bbox_pred_cpu[3*spatial_dim+idx]); 
            bbox.score = fg_score;
            cascade::ClipBBox(bbox, img_width, img_height);
    ...
    std::partial_sort(bboxes.begin(), bboxes.begin() + keep_num,
          bboxes.end(), cascade::SortBBoxDescend);
    bboxes.resize(keep_num);

    //NMS
    vector<cascade::BBox> new_bboxes;
    new_bboxes = cascade::ApplyNMS(bboxes, iou_thr_, true, nms_type_);
  ...
  if (num_batch_boxes <= 0) {
    ...
    for (int i = 0; i < batch_boxes.size(); i++) {
      const vector<cascade::BBox> bboxes = batch_boxes[i];
