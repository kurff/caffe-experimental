...
void ProposalTargetLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < num_rois; i++) {
    ...
    for (int j = 0; j < num_gt_boxes; j++) {
      ...
      exist_gt = true;
      float op = cascade::JaccardOverlap(rois_boxes[i][1], rois_boxes[i][2], rois_boxes[i][3], rois_boxes[i][4],
  ...
  if (num_keep_rois == 0) {
    ...
    while (backup_boxes.size() < num_backup) {
      ...
      for (int j = 0; j < num_gt_boxes; j++) {
        ...
        float op = cascade::JaccardOverlap(Dtype(bb_x), Dtype(bb_y), Dtype(bb_width), Dtype(bb_height), 
                gt_boxes[j][1], gt_boxes[j][2], gt_boxes[j][3], gt_boxes[j][4], "IOU"); 
