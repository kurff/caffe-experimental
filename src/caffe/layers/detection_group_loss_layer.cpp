...
void DetectionGroupLossLayer<Dtype>::Forward_cpu(
  ...
  // encode gt bbox
  cascade::EncodeGroupBBoxes(gt_bbox_.cpu_data(), gt_label_data, num, group_num_, width, 
          height, bbox_mean_.cpu_data(), bbox_std_.cpu_data(), field_ws_.cpu_data(), 
          field_hs_.cpu_data(), stride_, gt_bbox_.mutable_cpu_diff());
  
  // bound predictions
  if (do_bound_bbox_) {
    cascade::BoundBBoxPreds(bottom[1]->mutable_cpu_data(), num*group_num_, bbox_dim, 
