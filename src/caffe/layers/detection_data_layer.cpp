...
void DetectionDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  do {
    ...
    vector<cascade::BBox> roni_windows;
    infile >> num_roni_windows;
    for (int i = 0; i < num_roni_windows; ++i) {
      int x1, y1, x2, y2;      
      infile >> x1 >> y1 >> x2 >> y2;
      cascade::BBox roni_window = cascade::InitBBox(x1,y1,x2,y2);
...
void DetectionDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
      ...
      vector<cascade::BBox> windows = windows_[image_window_index];
      vector<cascade::BBox> roni_windows = roni_windows_[image_window_index];
      ...
      for (int nn = 0; nn < label_blob_num_; nn++) {
        ...
        for (int h = 0; h < label_height; ++h) {
          for (int w = 0; w < label_width; ++w) {
            ...
            cascade::BBox anchor_bbox = cascade::InitBBox(xx1,yy1,xx2,yy2);
            ...
            if (detection_data_param.has_anchor_ignore_margin_ratio()) {
              ...
              cascade::BBox image_bbox = cascade::InitBBox(src_offset_x,src_offset_y,
                      src_offset_x+copy_width,src_offset_y+copy_height);
              const float ignore_iou_thr = detection_data_param.anchor_ignore_iou_thr();
              float valid_iou = cascade::JaccardOverlap(anchor_bbox, image_bbox, "IOFU");
            ...
            for (int ww = 0; ww < roni_windows.size(); ++ww) {
              float iou = cascade::JaccardOverlap(anchor_bbox, roni_windows[ww], "IOFU");
              sum_ignore_iou += iou;
            }

            int match_idx = -1; float max_iou = 0;
            for (int ww = 0; ww < windows.size(); ++ww) {
              Dtype iou = cascade::JaccardOverlap(windows[ww], anchor_bbox, "IOU");
