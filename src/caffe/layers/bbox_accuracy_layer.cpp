...
void BboxAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < num; i++) {
    ...
    Dtype iou = cascade::JaccardOverlap(tx,ty,tw,th,gt_x,gt_y,gt_w,gt_h,"IOU");
    after_avg_iou += iou;
    Dtype roi_iou = cascade::JaccardOverlap(roi_x,roi_y,roi_w,roi_h,gt_x,gt_y,gt_w,gt_h,"IOU");
