...
void FrcnnOutputLayer<Dtype>::Forward_cpu(
  ...
  vector<cascade::BBox> prior_bboxes;
  ...
  cascade::DecodeBBoxesWithPrior(bbox_data,prior_bboxes,bbox_dim,bbox_mean_.cpu_data(),
          bbox_std_.cpu_data(),bbox_pred_data);
         
  int num_all_boxes = 0;
  vector<vector<vector<cascade::BBox> > > all_bboxes(num);
  for (int i = 0; i < num; i++) { 
    vector<vector<cascade::BBox> > cls_bboxes(cls_num-1);
    int num_det_img = 0;
    for (int c = 1; c < cls_num; c++) {
      vector<cascade::BBox> bboxes;
      int bb_count = 0;
      for (int id = 0; id < instance_num; id++) {
        ...
        if (cls_score >= conf_thr) {
          ...
          cascade::ClipBBox(bbox, size_data[i*2+1], size_data[i*2]);
      ...
      vector<cascade::BBox> new_bboxes;
      //NMS, make sure the scores are already ranked
      new_bboxes = cascade::ApplyNMS(bboxes, iou_thr, true, nms_type);
    ...
    if (max_det_num > 0 && num_det_img > max_det_num) {
      ...
      std::sort(score_index_pairs.begin(), score_index_pairs.end(),
                cascade::SortScorePairDescend<pair<int, int> >);
      score_index_pairs.resize(max_det_num);
      // Store the new indices.
      vector<vector<cascade::BBox> > new_cls_bboxes(cls_num-1);
      for (int j = 0; j < score_index_pairs.size(); ++j) {
        int label = score_index_pairs[j].second.first;
        int idx = score_index_pairs[j].second.second;
        cascade::BBox bbox = cls_bboxes[label-1][idx];
  ...
  for (int i = 0; i < num; ++i) {
    ...
    const vector<vector<cascade::BBox> > cls_bboxes = all_bboxes[i];
    for (int c = 1; c < cls_num; c++) {
      const vector<cascade::BBox> bboxes = cls_bboxes[c-1];
      for (int j = 0; j < bboxes.size(); j++) {
        ...
        if (need_save_) {
          cascade::BBox out_bbox = bboxes[j];
          ...
          cascade::AffineBBox(out_bbox,w_ratio,h_ratio,0,0);
