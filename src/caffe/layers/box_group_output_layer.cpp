...
void BoxGroupOutputLayer<Dtype>::Forward_cpu(
  ...
  for (int j = 0; j < bottom_num_; j++) {
    ...
    cascade::DecodeGroupBBoxes(bbox_data,num,group_nums_[j],bbox_dim,width,height,
      ...
      for (int k = 0; k < group_num; k++) {
        ...
        for (int id = 0; id < spatial_dim; id++) {
          ...
          if (fg_score >= fg_thr_) {
            ...
            cascade::BBox bbox = cascade::InitBBox(bbox_pred_data[idx], bbox_pred_data[spatial_dim+idx], 
