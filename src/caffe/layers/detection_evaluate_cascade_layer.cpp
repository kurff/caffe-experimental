...
#include "caffe/layers/detection_evaluate_cascade_layer.hpp"
#include "caffe/util/bbox_util_cascade.hpp"

namespace caffe {

template <typename Dtype>
void DetectionEvaluateCascadeLayer<Dtype>::LayerSetUp(
      const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  const DetectionEvaluateCascadeParameter& detection_evaluate_param =
      this->layer_param_.detection_evaluate_cascade_param();
...
template <typename Dtype>
void DetectionEvaluateCascadeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void DetectionEvaluateCascadeLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  const Dtype* det_data = bottom[0]->cpu_data();
  ...
  map<int, cascade::LabelBBox> all_detections;
  cascade::GetDetectionResults(det_data, bottom[0]->num(), bottom[0]->channels(), 
          &all_detections);

  // Retrieve all ground truth (including difficult ones).
  map<int, cascade::LabelBBox> all_gt_bboxes;
  cascade::GetGroundTruth(gt_data, bottom[1]->num(), bottom[1]->channels(), true, 
          &all_gt_bboxes);
  ...
  map<int, int> num_pos;
  for (map<int, cascade::LabelBBox>::iterator it = all_gt_bboxes.begin();
       it != all_gt_bboxes.end(); ++it) {
    for (cascade::LabelBBox::iterator iit = it->second.begin(); iit != it->second.end();
         ++iit) {
      int count = 0;
  ...
  for (map<int, cascade::LabelBBox>::iterator it = all_detections.begin(); 
    ...
    if (all_gt_bboxes.find(image_id) == all_gt_bboxes.end()) {
      ...
      cascade::LabelBBox& label_bboxes = all_gt_bboxes.find(image_id)->second;
      for (cascade::LabelBBox::iterator iit = detections.begin();
           iit != detections.end(); ++iit) {
        int label = iit->first;
        if (label == -1) {
        ...
        vector<cascade::BBox>& bboxes = iit->second;
        if (label_bboxes.find(label) == label_bboxes.end()) {
          ...
          vector<cascade::BBox>& gt_bboxes = label_bboxes.find(label)->second;
          // scale ground truth and detections to actual values
          cascade::AffineBBoxes(gt_bboxes,w_ratio,h_ratio,0,0);
          cascade::AffineBBoxes(bboxes,w_ratio,h_ratio,0,0);
          
          vector<bool> visited(gt_bboxes.size(), false);
          // Sort detections in descend order based on scores.
          std::sort(bboxes.begin(), bboxes.end(), cascade::SortBBoxDescend);
          for (int i = 0; i < bboxes.size(); ++i) {
            top_data[num_det * 5] = image_id;
            top_data[num_det * 5 + 1] = label;
            top_data[num_det * 5 + 2] = bboxes[i].score;
            ...
            for (int j = 0; j < gt_bboxes.size(); ++j) {
              float overlap = cascade::JaccardOverlap(bboxes[i], gt_bboxes[j],"IOU");
...
INSTANTIATE_CLASS(DetectionEvaluateCascadeLayer);
REGISTER_LAYER_CLASS(DetectionEvaluateCascade);
