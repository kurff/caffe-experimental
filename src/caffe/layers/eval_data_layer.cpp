...
void EvalDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
      ...
      vector<cascade::BBox> windows = windows_[image_index];
      vector<cascade::BBox> roni_windows = roni_windows_[image_index];
      ...
      if (resize_width != img_width || resize_height != img_height) {
        ...
        cascade::AffineBBoxes(windows,width_factor,height_factor,0,0);
        cascade::AffineBBoxes(roni_windows,width_factor,height_factor,0,0);
      }
