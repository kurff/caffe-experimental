...
bool ReadProtoFromTextFileEncrypt(Security* security, const char* filename, Message* proto){
  ...
  if(ifs.is_open()){
    std::string encrypt_content =std::string((std::istreambuf_iterator<char>(ifs) ),
                       (std::istreambuf_iterator<char>()) );
    ifs.close();
    std::string decrypt_content;
    security->decrypt(encrypt_content, decrypt_content);
    bool success = google::protobuf::TextFormat::ParseFromString( decrypt_content, proto);
...
bool ReadProtoFromBinaryFileEncrypt(Security* security, const char* filename, Message* proto){
  ...
  if(ifs.is_open()){
    ...
    std::string decrypt_content;
    security->decrypt(encrypt_content, decrypt_content);
    bool success = proto->ParseFromString(decrypt_content);
  ...
  if (cv_img.data) {
    if (encoding.size()) {
      if ( (cv_img.channels() == 3) == is_color && !height && !width &&
        ...
        datum->set_height(cv_img.rows);
        datum->set_width(cv_img.cols);
        return ReadFileToDatum(filename, label[0], datum);
      }
      EncodeCVMatToDatum(cv_img, encoding, datum);
    ...
    CVMatToDatum(cv_img, datum);
    assert(label[0]>=-1&&label[1]<=1);
    //assert(label)   
    for(int i =0;  i< label.size(); ++ i){
      datum->add_attribute(label[i]+1);
    }
    return true;
  } else {
    return false;
